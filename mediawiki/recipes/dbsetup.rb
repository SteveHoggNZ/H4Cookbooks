node[:deploy].each do |app_name, deploy|
  execute "mysql-setup" do
    command "cat mediawiki.sql.gz | gzip -d | /usr/bin/mysql -h#{deploy[:database][:host]} -u#{deploy[:database][:username]} -p#{deploy[:database][:password]} #{deploy[:database][:database]}"
